...
PGPASSWORD="$SHRINE_ONT_PASSWORD" psql  -h "$I2B2_DB_HOST" -d "$POSTGRES_DB" -U "$SHRINE_ONT_USER"  -f /opt/sql/ontology_create_tables.sql
PGPASSWORD="$SHRINE_ONT_PASSWORD" psql  -h "$I2B2_DB_HOST" -d "$POSTGRES_DB" -U "$SHRINE_ONT_USER"  -f /opt/sql/demoontology.sql	> /dev/null
PGPASSWORD="$SHRINE_ONT_PASSWORD" psql  -h "$I2B2_DB_HOST" -d "$POSTGRES_DB" -U "$SHRINE_ONT_USER"  -f /opt/sql/schemes4shineont.sql

PGPASSWORD="$SHRINE_ONT_PASSWORD" psql  -h "$I2B2_DB_HOST" -d "$POSTGRES_DB" -U "$SHRINE_ONT_USER" -c "INSERT into shrine_ont.TABLE_ACCESS ( C_TABLE_CD, C_TABLE_NAME, C_PROTECTED_ACCESS, C_HLEVEL, C_NAME, C_FULLNAME, C_SYNONYM_CD, C_VISUALATTRIBUTES, C_TOOLTIP, C_FACTTABLECOLUMN, C_DIMTABLENAME, C_COLUMNNAME, C_COLUMNDATATYPE, C_DIMCODE, C_OPERATOR) values ( 'SHRINE', 'SHRINE', 'N', 0, 'SHRINE Ontology', '\SHRINE\', 'N', 'CA', 'SHRINE Ontology', 'concept_cd', 'concept_dimension', 'concept_path', 'T', '\SHRINE\', 'LIKE');"
